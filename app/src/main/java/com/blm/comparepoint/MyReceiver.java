...
public class MyReceiver extends BroadcastReceiver {
    ...
    public void onReceive(Context context, Intent intent) {
        ...
        if (JPushInterface.ACTION_REGISTRATION_ID.equals(intent.getAction())) {
            ...
            //打开自定义的Activity
            context.startActivity(new Intent(context, NotifyDetial.class).setFlags(Intent.FLAG_ACTIVITY_NEW_TASK).putExtra("data", bundle).putExtra("isPush", true));
    ...
    private void processCustomMessage(Context context, Bundle bundle) {
        if (Home.isForeground) {
            ...
            Intent msgIntent = new Intent(Home.MESSAGE_RECEIVED_ACTION);
            msgIntent.putExtra(Home.KEY_MESSAGE, message);
            if (!TextUtils.isEmpty(extras)) {
                try {
                    JSONObject extraJson = new JSONObject(extras);
                    if (null != extraJson && extraJson.length() > 0) {
                        msgIntent.putExtra(Home.KEY_EXTRAS, extras);
                    }
                } catch (JSONException e) {
